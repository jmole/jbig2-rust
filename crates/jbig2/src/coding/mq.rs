...
impl MqEncoder {
    ...
    /// Reset the encoder state for another stream while retaining the output
    /// buffer allocation.
    pub fn reset(&mut self, capacity_hint: usize) {
        ...
        self.b = 0;
        self.first = true;
        if capacity_hint > self.out.capacity() {
    ...
    /// encode. Used by the page encoder to bounce one allocation between the
    /// generic-region hot path and the segment writer.
    ...
    /// Flush into a caller-owned buffer so the allocation can be reused by the
    /// next encode.
...
impl<'a> MqDecoder<'a> {
    ...
    pub fn decode<C: Into<ContextIndex>>(&mut self, cxs: &mut MqContexts, cx: C) -> u8 {
        ...
        let renorm_needed = (self.a_reg & 0x8000) == 0;
        let mps_exchange = swap && self.a_reg < qe;
        let is_lps = if swap {
            !mps_exchange
        } else {
            renorm_needed && self.a_reg < qe
        };
        ...
        if is_lps {
            *state = lps_state;
        } else if swap || renorm_needed {
            *state = mps_state;
        }

        self.renorm();
        u8::from((if is_lps { lps_bit } else { mps_bit }) != 0)
    }
...
mod tests {
    ...
    impl<'a> MqDecoderLegacy<'a> {
        ...
        fn decode(&mut self, cxs: &mut MqContexts, cx: usize) -> u8 {
        ...
        fn state(&self) -> (u32, u32, i32, u8, usize) {
            (self.a_reg, self.c_reg, self.ct, self.b, self.pos)
        }
    }
    ...
    proptest! {
        #[test]
        fn decoder_matches_legacy(
            decisions in prop::collection::vec((0usize..16usize, 0u8..2u8), 1..4096),
            capacity_hint in 0usize..256usize,
        ) {
            let out = encode_decisions(&decisions, capacity_hint);
            let mut fast_cx = MqContexts::new(16);
            let mut legacy_cx = MqContexts::new(16);
            let mut fast = MqDecoder::new(&out);
            let mut legacy = MqDecoderLegacy::new(&out);

            for (idx, &(cx, bit)) in decisions.iter().enumerate() {
                let got_fast = fast.decode(&mut fast_cx, cx);
                let got_legacy = legacy.decode(&mut legacy_cx, cx);
                prop_assert_eq!(got_fast, bit, "fast mismatch at decision {}", idx);
                prop_assert_eq!(got_legacy, bit, "legacy mismatch at decision {}", idx);
                prop_assert_eq!(got_fast, got_legacy, "decoder disagreement at decision {}", idx);
                prop_assert_eq!(fast_cx.as_mut_slice(), legacy_cx.as_mut_slice());
                prop_assert_eq!(fast.state(), legacy.state());
            }
        }
    }
}

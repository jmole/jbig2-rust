...
impl DecodedPage {
    fn into_dynamic(self) -> DynamicImage {
        if let Some(rgb) = self.rgb_bitmap {
            return DynamicImage::ImageRgb8(
                RgbImage::from_raw(rgb.width(), rgb.height(), rgb.into_data())
                    .expect("RGB buffer sized width*height*3 by construction"),
            );
        }
        ...
        DynamicImage::ImageLuma8(
            GrayImage::from_raw(width, height, buf)
                .expect("grayscale buffer sized width*height by construction"),
        )
    }
}
...
impl<R: Read + Seek> Jbig2Decoder<R> {
    ...
    /// Decode a single page as an image-rs image. `page_number` is 1-based to
    /// match the JBIG2 specification.
    pub fn decode_page(&mut self, page_number: u32) -> Jbig2Result<DynamicImage> {
        self.decode_page_raw(page_number)
            .map(DecodedPage::into_dynamic)
    }

    /// Decode a single page into the crate's packed internal representation.
    pub fn decode_page_raw(&mut self, page_number: u32) -> Jbig2Result<DecodedPage> {

...
impl<R: Read + Seek> ImageDecoder for Jbig2ImageDecoder<R> {
    ...
    where
        Self: Sized,
    {
        let page = self.inner.decode_page_raw(1).map_err(jbig2_to_image)?;
...
#[allow(dead_code)]
pub(crate) fn decode_all_pages_gray(bytes: &[u8]) -> crate::Jbig2Result<Vec<image::GrayImage>> {
    let mut inner = Jbig2Decoder::new(Cursor::new(bytes))?;
    let n = inner.num_pages();
    let mut out = Vec::with_capacity(n as usize);
    for p in 1..=n {
        let page = inner.decode_page_raw(p)?;
        let bm = page.bitmap;
        let width = bm.width();

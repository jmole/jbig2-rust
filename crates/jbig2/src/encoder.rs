...
fn bitmap_from_gray_image(image: &GrayImage) -> Jbig2Result<Bitmap> {
    let mut bitmap = Bitmap::new(image.width(), image.height())?;
    for y in 0..image.height() {
        let row = bitmap.row_mut(y as usize);
...
impl<W: Write> Jbig2Encoder<W> {
    ...
    #[allow(dead_code)]
    pub(crate) fn with_num_pages(mut self, n: u32) -> Self {
        self.total_pages = n;
        self
    }
    ...
    /// Encode one grayscale page.
    ///
    /// Pixels below 128 are treated as JBIG2 ink (`1`); pixels 128 and above
    /// are treated as paper (`0`).
    pub fn write_page(&mut self, image: &GrayImage) -> Jbig2Result<()> {
        let bitmap = bitmap_from_gray_image(image)?;
        self.write_bitmap_internal(&bitmap)
    }

    /// Encode an already-packed internal bitmap. This is for repository
    /// workbench tools only; public consumers should use [`write_page`].
    #[cfg(feature = "__workbench")]
    pub fn write_bitmap(&mut self, bitmap: &Bitmap) -> Jbig2Result<()> {
        self.write_bitmap_internal(bitmap)
    }

    fn write_bitmap_internal(&mut self, bitmap: &Bitmap) -> Jbig2Result<()> {
    ...
    fn encode_page_to_vec(bitmap: &Bitmap, mut cfg: EncoderConfig) -> Jbig2Result<Vec<u8>> {
        ...
        encoder.write_bitmap_internal(bitmap)?;
        encoder.finish()
    }

    /// Encode one page through the symbol-dictionary + text-region path.
    pub(crate) fn write_page_symbols(
        &mut self,
        bitmap: &Bitmap,
        coding: SymbolCoding,
    ) -> Jbig2Result<()> {
    ...
    #[allow(dead_code)]
    pub(crate) fn write_page_halftone(
        &mut self,
        patterns: &[Bitmap],
        gray_values: &[u32],
...
mod tests {
    ...
    fn fast_preset_round_trip() {
        ...
        enc.write_bitmap_internal(&bm).unwrap();
        enc.finish().unwrap();

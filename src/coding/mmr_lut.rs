...
fn build_run_lut(white: bool) -> Box<[RunLut; 1 << 13]> {
    ...
    for (idx, &(code, len)) in term.iter().enumerate() {
        fill_run_entries(
            ...
            code,
            len,
            RunLut { len, kind: 1, run: idx as u16 },
        );
    }
...
impl<'a> MmrBitBuf<'a> {
    ...
    pub fn new(buf: &'a [u8]) -> Self {
        let mut s = Self { buf, pos: 0, cache: 0, cache_bits: 0 };
        s.fill();
        s
    }
...
mod tests {
    ...
    fn cross_check_lines(lines: &[Vec<bool>]) {
        ...
        for (i, line) in lines.iter().enumerate() {
            ...
            assert_eq!(&expected, line, "slow path disagrees with source on line {i}");
        }
    }

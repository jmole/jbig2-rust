    ...
    while ninst < header.num_instances {
        ...
        loop {
            if first {
                ...
                let ids = decode_integer(dec, cxs, IADS);
                match ids {
                    None => break,
                    Some(v) => {
            ...
            // Reference quirk (Jbig2DEC.cpp `SymbolAggregate_MQDec`):
            // when SBREFINE = 1, the encoder skips IAIT entirely AND keeps
            // Cur_T pinned at 0 (the strip_t machinery is essentially
            // ignored). We mirror that to stay byte-compatible with the
            // conformance files generated by the reference encoder.
            ...
            let ib: &Bitmap = if r_flag == 0 {
                ...
                // Per spec 6.4.11: per-instance refinement body.
                ...
                // Spec 6.4.11 step (e): the refinement-region shift is
                // `floor((RDW)/2) + RDX` and `floor((RDH)/2) + RDY`.
...
pub fn decode_text_region_colour_with_contexts(
    ...
    let s0 = decode_integer(&mut dec, cxs, IADT).ok_or(
        Jbig2Error::InvalidHuffman("text region: IADT initial returned OOB"),
    )?;
    let mut strip_t: i64 = -(s0 as i64) * sb_strips as i64;
    let mut first_s: i64 = 0;
    ...
    while ninst < header.num_instances {
        let dt = decode_integer(&mut dec, cxs, IADT).ok_or(
            Jbig2Error::InvalidHuffman("text region: strip IADT returned OOB"),
        )?;
        strip_t = strip_t.saturating_add((dt as i64).saturating_mul(sb_strips as i64));
        ...
        loop {
            if first {
                let dfs = decode_integer(&mut dec, cxs, IAFS).ok_or(
                    Jbig2Error::InvalidHuffman("text region: IAFS returned OOB"),
                )?;
                first_s = first_s.saturating_add(dfs as i64);
                cur_s = first_s;
                ...
                match decode_integer(&mut dec, cxs, IADS) {
                    None => break,
                    Some(v) => {
                        cur_s = cur_s.saturating_add(v as i64 + header.sbds_offset as i64);
                    }
            ...
            let t_abs = if header.sbrefine {
                ...
                let current_t = if sb_strips != 1 {
                    decode_integer(&mut dec, cxs, IAIT).ok_or(Jbig2Error::InvalidHuffman(
                        "text region: IAIT returned OOB",
                    ))? as i64
                } else {
                    0
            ...
            let id = decode_iaid(&mut dec, cxs, IAID, code_len);
            if (id as usize) >= sbsyms.len() {
                return Err(Jbig2Error::OutOfRange("text region: symbol ID out of range"));
            }
            let r_flag = if header.sbrefine {
                decode_integer(&mut dec, cxs, IARI).ok_or(Jbig2Error::InvalidHuffman(
                    "text region: IARI returned OOB",
                ))?
            } else {
                0
            ...
            let ib: &Bitmap = if r_flag == 0 {
                ...
                let rdw = decode_integer(&mut dec, cxs, IARDW).ok_or(
                    Jbig2Error::InvalidHuffman("text region: IARDW returned OOB"),
                )?;
                let rdh = decode_integer(&mut dec, cxs, IARDH).ok_or(
                    Jbig2Error::InvalidHuffman("text region: IARDH returned OOB"),
                )?;
                let rdx = decode_integer(&mut dec, cxs, IARDX).ok_or(
                    Jbig2Error::InvalidHuffman("text region: IARDX returned OOB"),
                )?;
                let rdy = decode_integer(&mut dec, cxs, IARDY).ok_or(
                    Jbig2Error::InvalidHuffman("text region: IARDY returned OOB"),
                )?;
                let ref_bm = sbsyms[id as usize];
                let ib_w = ref_bm.width() as i32 + rdw;
...
pub fn encode_text_region(
    ...
    encode_text_region_body(&mut enc, &mut cxs, header, instances, symbols)?;
    Ok(enc.finish())
}
...
fn blit(region: &mut Bitmap, ib: &Bitmap, x0: i32, y0: i32, op: CombinationOp) {
    ...
    region.composite(ib, x0, y0, combop_to_blit(op));
}
